...
def CreatePlot(myfile):
    ...
    #f = go.Figure(data=[go.Contour(z=XYZMatrix,colorscale='Viridis')]) #построить поверхность по данным в матрице
    ...
    f.update_layout(scene=dict(xaxis=dict(nticks=int(stepx*5),range=[startx,lastx*stepx]),yaxis=dict(nticks=int(stepy*5),range=[starty,lasty*stepy])))
